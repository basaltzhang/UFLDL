...
diff = sum(sum((a3 - data) .^ 2, 2));
sumW1 = sum(sum(W1 .^ 2, 2));
sumW2 = sum(sum(W2 .^ 2, 2));
rho = sum(a2, 2) / size(data, 2);

 
cost = (1 / (2 * size(data, 2))) * diff + lambda * (sumW1 + sumW2) / 2 ...
     + beta * sum(sparsityParam * log(sparsityParam ./ rho) ...
     + (1 - sparsityParam) * log((1 - sparsityParam) ./ (1 - rho)));

delta3 = - (data - a3);
delta2 = (W2' * delta3 ...
    + beta * repmat((-sparsityParam ./ rho + (1 - sparsityParam) ./ (1 - rho)), 1, size(data, 2))) ...
    .* a2 .* (1 - a2);
